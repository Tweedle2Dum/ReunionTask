  ...
  const applyFilters = () => {
    ...
    if (filterTerm.location) {
      filteredArray = filteredArray.filter(
        (item) => item.location === filterTerm.location
      );
    }
    if (filterTerm.priceMin) {
      filteredArray = filteredArray.filter(
        (item) => item.price >= parseInt(filterTerm.priceMin)
      );
    }
    if (filterTerm.priceMax) {
      filteredArray = filteredArray.filter(
        (item) => item.price <= parseInt(filterTerm.priceMax)
      );
    }
    if (filterTerm.date) {
    ...
    if (filterTerm.propertyType) {
      filteredArray = filteredArray.filter(
        (item) => item.propertyType === filterTerm.propertyType
      );
    }
    ...
    >
      <Box sx={{ minWidth: "240px" }}>
        ...
        <FormControl fullWidth>
          ...
          <Select
            ...
            label="Location"
            onChange={(e) =>
              setFilterTerm({ ...filterTerm, location: e.target.value })
            }
          >
            ...
            {data &&
              data.map((obj) => {
                return (
                  <MenuItem key={obj.id} value={obj.location}>
                    {obj.location}
                  </MenuItem>
                );
              })}
      ...
      <Box display={"flex"}>
        <TextField
          ...
          onChange={(e) =>
            setFilterTerm({ ...filterTerm, priceMin: e.target.value })
          }
        />
        <TextField
          label="Max"
          ...
          onChange={(e) =>
            setFilterTerm({ ...filterTerm, priceMax: e.target.value })
          }
        />
      </Box>
      ...
      <Box sx={{ minWidth: "150px" }}>
        ...
        <FormControl fullWidth>
          ...
          <Select
            ...
            label="type"
            onChange={(e) =>
              setFilterTerm({ ...filterTerm, propertyType: e.target.value })
            }
          >
